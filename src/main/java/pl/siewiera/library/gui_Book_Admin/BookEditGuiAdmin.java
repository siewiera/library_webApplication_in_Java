...
@Route("edit_book_Admin")
@StyleSheet("/css/style.css")
public class BookEditGuiAdmin extends VerticalLayout {
    String tytul = "Edycja danych książki";
    String książka = "";

    //lista kategori książek
    List<String> bookCategory1 = new ArrayList<>();

    @Autowired
    public BookEditGuiAdmin(BookRepo bookRepo) {
        ...
        GridPro.Column<Book> editorColumn = gridPro
...
//
        ...
        bookCategory1.add("FANTASTYKA");
        bookCategory1.add("HORROR");
        bookCategory1.add("KRYMINAŁ");
        bookCategory1.add("BAJKA");
        ...
        title.setEditorComponent(title2);

        TextField name_author2 = new TextField();
        name_author2.setSizeFull();
        binder.forField(name_author2)
                .withValidator(new StringLengthValidator("Liczba znaków musi być między 2, a 20", 2, 20))
                .withStatusLabel(validationStatus).bind("name_author");
        name_author.setEditorComponent(name_author2);

        TextField surname_author2 = new TextField();
        surname_author2.setSizeFull();
        binder.forField(surname_author2)
                .withValidator(new StringLengthValidator("Liczba znaków musi być między 2, a 20", 2, 20))
                .withStatusLabel(validationStatus).bind("surname_author");
        surname_author.setEditorComponent(surname_author2);

        ComboBox<BookCategory> bookCategory2 = new ComboBox<>("", BookCategory.values());
        bookCategory2.setSizeFull();
        binder.forField(bookCategory2)
                .withStatusLabel(validationStatus).bind("bookCategory");
        bookCategory.setEditorComponent(bookCategory2);

        DatePicker releaseDate2 = new DatePicker();
        releaseDate2.setSizeFull();
        binder.forField(releaseDate2)
                .withStatusLabel(validationStatus).bind("releaseDate");
        releaseDate.setEditorComponent(releaseDate2);

        TextField edition2 = new TextField();
        edition2.setSizeFull();
        binder.forField(edition2)
                .withValidator(new StringLengthValidator("Liczba znaków musi być między 2, a 20", 2, 20))
                .withStatusLabel(validationStatus).bind("edition");
        edition.setEditorComponent(edition2);

        TextField description2 = new TextField();
        description2.setSizeFull();
        binder.forField(description2)
                .withValidator(new StringLengthValidator("Liczba znaków musi być między 2, a 200", 2, 200))
                .withStatusLabel(validationStatus).bind("description");
        description.setEditorComponent(description2);

        TextField quantity2 = new TextField();
        quantity2.setSizeFull();
        binder.forField(quantity2)
                .withConverter(new StringToIntegerConverter("Ilość musi być liczbą"))
                .withStatusLabel(validationStatus).bind("quantity");
        quantity.setEditorComponent(quantity2);
