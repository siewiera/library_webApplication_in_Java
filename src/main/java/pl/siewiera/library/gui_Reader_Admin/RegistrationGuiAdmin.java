...
@Route("registration_Admin")
@StyleSheet("/css/style.css")
public class RegistrationGuiAdmin extends VerticalLayout {
    String tytul = "Rejestracja czytelnika";
    ...
    @Autowired
    public RegistrationGuiAdmin(ReaderRepo readerRepo) {
        ...
        emailField.getStyle().set("width", "40%").set("margin-left", "0px");
        ...
        passwordField.getStyle().set("width", "40%").set("margin-left", "30px");
        ...
        textFieldName.getStyle().set("width", "30%").set("margin-left", "0px");
        ...
        textFieldSurname.getStyle().set("width", "30%").set("margin-left", "30px");
        ...
        datePicker.getStyle().set("width", "30%").set("margin-left", "60px");
        datePicker.setPlaceholder("Data urodzenia");

        TextField numberPhoneField = new TextField();
        numberPhoneField.getStyle().set("width", "30%").set("margin-left", "90px");
...
//                .bind(String.valueOf(reader.getPhoneNumber()));
                    ...
                    if (textFieldName.getValue().length() >= 2) {
                        if (textFieldSurname.getValue().length() >= 2) {
                            if (numberPhoneField.getValue().length() == 9) {
                                ...
                                reader.setPassword(passwordEncoder().encode(passwordField.getValue()));
                                ...
                                reader.setPhoneNumber(Integer.valueOf(numberPhoneField.getValue()));
                                reader.setImage(textFieldImage.getValue());
                                reader.setRole(role2.getValue());
                                if(reader.getImage().length()<=0){
                                    reader.setImage("https://external-content.duckduckgo.com/iu/?u=https%3A%2F%2Fthumbs.dreamstime.com%2Ft%2Fu%25C5%25BCytkownik-osoba-obrachunkowy-ikona-wektor-wype%25C5%2582niaj%25C4%2585cy-mieszkanie-znak-sta%25C5%2582y-piktogram-odizolowywaj%25C4%2585cy-na-bielu-95992530.jpg&f=1&nofb=1");
                                }
                                readerRepo.save(reader);
                                Notification notification = new Notification(
                                        "Dodano: " + textFieldName.getValue() + " " + textFieldSurname.getValue() + " do bazy czytelników!", 3000,
        ...
        zawartosc.add(textFieldName, textFieldSurname, datePicker, numberPhoneField, role2, textFieldImage);
