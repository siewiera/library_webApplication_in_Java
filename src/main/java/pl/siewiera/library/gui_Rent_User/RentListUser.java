...
@Route("list_rent_User")
@StyleSheet("/css/style.css")
public class RentListUser extends VerticalLayout {

    String tytul = "Historia wypożyczeń";
    String wypozyczenie = "";

    @Autowired
    public RentListUser(RentRepo rentRepo) {
        ...
        List<Rent> collect = rents.stream()
                .filter(rent -> rent.getReader().toString().equals(SecurityContextHolder
                        .getContext().getAuthentication().getPrincipal().toString()))
                .collect(Collectors.toList());
        Grid<Rent> gridPro = new Grid<>(Rent.class);
...
//stworzenie miejsca na filtry w nagłówku tabeli
        ...
        naglowek.add(tytul);
        menu.add(powrot);
        zawartosc.add(gridPro, bookField, quantityField, rent_dateField, return_dateField);
